 ...
 *   DATABASE_URL                  (Neon)
 *   R2_ACCOUNT_ID / _ACCESS_KEY_ID / _SECRET_ACCESS_KEY / _BUCKET / _PUBLIC_DOMAIN  (R2 object storage)
 */
"use node";
...
// R2 signed URLs are generated by the official AWS SDK (PutObjectCommand +
// getSignedUrl) — never hand-rolled SigV4. See convex/lib/r2presign.ts.
...
// ---------------------------------------------------------------------------
// product images — storage (R2) + metadata (Neon)
// ---------------------------------------------------------------------------
/**
 * Step 1 of upload: seller asks the backend for a signed R2 upload URL.
 * The browser then PUTs the file straight to R2 with the signed URL
 * (no binary bytes through our server). file type + max size are enforced
 * server-side in saveProductImage.
 */
export const getProductImageUploadIntent = action({
  args: { productId: v.string(), filename: v.string(), mimeType: v.string() },
  handler: async (ctx, args) => {
    ...
    if (!allowed.includes(ext) && !args.mimeType.startsWith("image/")) {
      throw new AppError("INVALID_INPUT", `ไฟล์รูปประเภท ${args.mimeType} ไม่ได้รับอนุญาต (รองรับ: ${ALLOWED_IMAGE_FORMATS})`);
    }
    ...
    return { productId: product.id, uploadUrl: upload.uploadUrl, objectKey, cdnUrl: upload.cdnUrl, expiresAt: upload.expiresAt };
  },
});
 ...
 * the canonical URL from the R2 object key, and only then inserts the
...
export const deleteProductImageAction = action({
  ...
  handler: async (ctx, args) => {
    ...
    // best-effort binary cleanup in R2 object storage
    if (removed.image.storageKey && isStorageConfigured()) {
      try {
        await deleteR2File(removed.image.storageKey);
      } catch (err) {
        console.error("[commerce] R2 storage delete failed (row removed anyway):", err);
      }
    }
    return getProduct(db, product.id);
  },
