...
export default function MerchantVelRepeatPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [sum, list] = await Promise.all([
        apiClient.get<Summary>('/velrepeat/merchant/summary'),
        apiClient.get<PackRow[]>(`/velrepeat/merchant/packs${qs}`),
      ]);
      setSummary(sum);
  ...
  const filtered = !q
    ? packs
    : packs.filter((p) => {
        const hay = [p.planCode, p.product?.name, p.user?.name, p.user?.email, p.id]
          .filter(Boolean)
          .join(' ')
          .toLowerCase();
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold text-slate-900">VelRepeat ของร้าน</h1>
          <p className="text-sm text-slate-500">
            ดูแพ็กที่ลูกค้าซื้อจากสินค้าของคุณ · เครดิตคงเหลือ · รอบส่งถัดไป
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {[
          { label: 'แพ็กทั้งหมด', value: summary?.total ?? '—' },
          { label: 'ใช้งานอยู่', value: summary?.active ?? '—', tone: 'text-emerald-700' },
          {
            label: 'เครดิตคงเหลือ (ชิ้น)',
          ...
          >
            ...
            <p className={`mt-1 text-xl font-bold ${c.tone ?? 'text-slate-900'}`}>{c.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <input
          ...
          placeholder="ค้นหาสินค้า / ลูกค้า / แผน..."
          className="ml-auto min-w-[180px] flex-1 rounded-md border border-slate-300 px-3 py-2 text-sm outline-none focus:border-teal-600 sm:max-w-xs"
        />
      </div>
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-12 text-center text-slate-400">
                    ยังไม่มีลูกค้าซื้อแพ็กจากร้านคุณ
                  </td>
                </tr>
              ) : (
                filtered.map((p) => (
