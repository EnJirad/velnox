...
export function CheckoutView() {
  ...
  const applySaved = useCallback((row: SavedAddress) => {
    ...
    setAddress({
      name: row.name,
      phone: row.phone,
      line: stripGeoFromText(row.addressLine),
      city: row.city || '',
      province: row.province,
      postal: row.postalCode,
    });
    setGeo(parseGeoFromText(row.addressLine));
  }, []);
  ...
  async function handlePlaceOrder() {
    ...
    setSubmitting(true);
    try {
      const addressLine = encodeGeoInText(address.line, geo);
                            ...
                            <div className="flex items-start justify-between gap-2">
                              <div>
                                ...
                                <p className="mt-1 text-xs text-slate-600">
                                  {stripGeoFromText(row.addressLine)}
                                </p>
