...
-- ----------------------------------------------------------------------------
-- Architecture v3: Neon owns commerce data. Convex = Intelligence + Realtime.
-- NEVER create a second source of truth for these entities.
--
-- This file represents the CURRENT COMPLETE DATABASE SCHEMA after applying
-- db/migrations/002 through 015. It is used by:
--   1. db/migrate.ts — applied first, then numbered migrations
--   2. db/run-sqleditor.sql — one-shot bootstrap (same content, idempotent)
--
-- Rules:
...
-- ============================================================================
-- 1. media — binary storage metadata (Cloudflare R2 is source of truth)
--     Neon stores metadata only; binary lives in R2.
--     Migrated from Cloudinary → R2 (migration 015).
-- ============================================================================
...
-- ============================================================================
-- 2. users — business attributes keyed by the Convex auth id (auth = Convex)
-- ============================================================================
...
-- ============================================================================
-- 3. user_profiles — extended profile (1:1 with users) (migration 002)
-- ============================================================================
...
-- ============================================================================
-- 4. sellers — merchant who opened a shop (User -> Seller -> Shop)
-- ============================================================================
CREATE TABLE IF NOT EXISTS sellers (
  ...
  rejection_reason     TEXT,                          -- migration 013
  refund_policy_limit  NUMERIC(6,4) NOT NULL DEFAULT 0.10,
  business_type        TEXT,                          -- migration 009: individual/company
  contact_phone        TEXT,                          -- migration 009
  contact_email        TEXT,                          -- migration 009
  deleted_at           TIMESTAMPTZ,                   -- migration 009
  created_at           TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- ============================================================================
-- 5. shops — one Seller can own several shops (marketplace-ready)
-- ============================================================================
...
-- ============================================================================
-- 6. categories — hierarchy (migration 003)
-- ============================================================================
...
-- ============================================================================
-- 7. products — catalog + CURRENT price only (never used for old orders)
-- ============================================================================
CREATE TABLE IF NOT EXISTS products (
  id                UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  shop_id           UUID NOT NULL REFERENCES shops (id) ON DELETE CASCADE,
  seller_id         UUID REFERENCES sellers (id),        -- migration 003
  category_id       UUID REFERENCES categories (id),      -- migration 003
  name              TEXT NOT NULL,
  description       TEXT,
  category          TEXT NOT NULL DEFAULT 'general'
                    CHECK (category IN ('general','food','daily','beauty','packaging','other')),
  slug              TEXT,                                 -- migration 003
  brand             TEXT,                                 -- migration 003
  unit              TEXT NOT NULL DEFAULT 'piece',
  price             NUMERIC(12,2) NOT NULL DEFAULT 0,
  compare_at_price  NUMERIC(12,2),                        -- migration 003
  currency          TEXT NOT NULL DEFAULT 'THB',
  product_type      TEXT NOT NULL DEFAULT 'physical'
                    CHECK (product_type IN ('one_time','velrepeat','service','digital','physical')),
  weight            NUMERIC(10,3),                        -- migration 003
  status            TEXT NOT NULL DEFAULT 'draft'
                    CHECK (status IN ('draft','pending_review','published','rejected','archived')),
  rejection_reason  TEXT,                                  -- migration 013
  supplier          TEXT,
  deleted_at        TIMESTAMPTZ,                           -- migration 003
  created_at        TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at        TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- ============================================================================
-- 8. product_variants — per-variant SKU/price/stock/image (migration 003)
-- ============================================================================
...
-- ============================================================================
-- 9. product_images — metadata ONLY; binary lives in Cloudflare R2
-- ============================================================================
CREATE TABLE IF NOT EXISTS product_images (
  id               UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  product_id       UUID NOT NULL REFERENCES products (id) ON DELETE CASCADE,
  variant_id       UUID REFERENCES product_variants (id) ON DELETE SET NULL,  -- migration 003
  url              TEXT NOT NULL,
  storage_provider TEXT NOT NULL DEFAULT 'r2',
  storage_key      TEXT,
  thumbnail_url    TEXT,                                                     -- migration 003
  alt              TEXT,
  sort_order       INTEGER NOT NULL DEFAULT 0,
  is_primary       BOOLEAN NOT NULL DEFAULT false,
  width            INTEGER,
  height           INTEGER,
  r2_object_key    TEXT,                                                     -- migration 015
  r2_cdn_url       TEXT,                                                     -- migration 015
  created_at       TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- ============================================================================
-- 10. inventory — stock is a SEPARATE entity from product
-- ============================================================================
CREATE TABLE IF NOT EXISTS inventory (
  id                UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  product_id        UUID NOT NULL REFERENCES products (id) ON DELETE CASCADE,
  variant_id        UUID REFERENCES product_variants (id) ON DELETE CASCADE,  -- migration 003
  shop_id           UUID NOT NULL REFERENCES shops (id) ON DELETE CASCADE,
...
-- ============================================================================
-- 11. addresses — customer saved addresses
-- ============================================================================
CREATE TABLE IF NOT EXISTS addresses (
  ...
  longitude      NUMERIC(10,7)
                 CHECK (longitude IS NULL OR (longitude >= -180 AND longitude <= 180)),
  subdistrict    TEXT,                                  -- migration 002
  district       TEXT,                                  -- migration 002
  province       TEXT,                                  -- migration 002
  place_id       TEXT,                                  -- migration 002
  is_default     BOOLEAN NOT NULL DEFAULT false,
  deleted_at     TIMESTAMPTZ,                           -- migration 002
  created_at     TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at     TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- ============================================================================
-- 12. carts — one active cart per user (migration 004)
-- ============================================================================
...
-- ============================================================================
-- 13. cart_items — multi-seller items with price snapshot (migration 004)
-- ============================================================================
...
-- ============================================================================
-- 15. orders — order header with frozen address snapshot + statuses
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS orders (
  ...
  parent_order_id  UUID REFERENCES orders (id),                             -- migration 005
  seller_id        UUID REFERENCES sellers (id),                            -- migration 005
  shop_id          UUID REFERENCES shops (id),                              -- migration 005
  status           TEXT NOT NULL DEFAULT 'pending'
  ...
  tax              NUMERIC(12,2) NOT NULL DEFAULT 0,                        -- migration 005
...
-- ============================================================================
-- 16. order_items — line-level SNAPSHOT (never re-price from products)
-- ============================================================================
CREATE TABLE IF NOT EXISTS order_items (
  id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  order_id        UUID NOT NULL REFERENCES orders (id) ON DELETE CASCADE,
  product_id      UUID NOT NULL,
  variant_id      UUID,                                                      -- migration 005
  variant_name    TEXT,                                                      -- migration 005
  sku             TEXT,                                                      -- migration 005
  ...
  discount        NUMERIC(12,2) NOT NULL DEFAULT 0,                          -- migration 005
  commission_rate NUMERIC(6,4) NOT NULL DEFAULT 0.03,
  created_at      TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- ============================================================================
-- 17. payments — one order can have several payment attempts
-- ============================================================================
CREATE TABLE IF NOT EXISTS payments (
  id           UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  order_id     UUID NOT NULL REFERENCES orders (id) ON DELETE CASCADE,
  user_id      UUID REFERENCES users (id),                                  -- migration 005
  amount       NUMERIC(12,2) NOT NULL,
  currency     TEXT NOT NULL DEFAULT 'THB',
  method       TEXT NOT NULL CHECK (method IN ('cod','transfer','card','promptpay','wallet')),
  provider     TEXT,                                                        -- migration 005
  status       TEXT NOT NULL DEFAULT 'pending'
...
-- ============================================================================
-- 18. payment_transactions — provider-level journal per payment (migration 005)
-- ============================================================================
...
-- ============================================================================
-- 19. refunds — return / dispute records
-- ============================================================================
CREATE TABLE IF NOT EXISTS refunds (
  ...
  amount            NUMERIC(12,2) NOT NULL,
  currency          TEXT NOT NULL DEFAULT 'THB',                            -- migration 005
  reason            TEXT,
  status            TEXT NOT NULL DEFAULT 'requested'
                    CHECK (status IN ('requested','approved','processed','rejected')),
  provider_refund_id TEXT,                                                   -- migration 005
  completed_at      TIMESTAMPTZ,                                            -- migration 005
  created_at        TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- ============================================================================
-- 20. commissions — platform fee per line item (snapshot), voided on refund
-- ============================================================================
...
-- ============================================================================
-- 21. settlements — periodic payout summary per seller
-- ============================================================================
...
-- ============================================================================
-- 22. subscriptions — VelRepeat commerce source of truth
-- ============================================================================
CREATE TABLE IF NOT EXISTS subscriptions (
  ...
  shop_id             UUID NOT NULL REFERENCES shops (id) ON DELETE CASCADE,
  seller_id           UUID NOT NULL,
  variant_id          UUID REFERENCES product_variants (id) ON DELETE SET NULL,  -- migration 007
  quantity            INTEGER NOT NULL
                      CHECK (quantity > 0),
  ...
  last_order_date     DATE,                                                   -- migration 007
  payment_method      TEXT,                                                   -- migration 007
  shipping_address_id UUID REFERENCES addresses (id) ON DELETE SET NULL,      -- migration 007
  status              TEXT NOT NULL DEFAULT 'active'
...
-- ============================================================================
-- 23. reviews — product/store reviews (migration 007)
-- ============================================================================
...
-- ============================================================================
-- 24. velrepeat_orders — auto-placed orders linked to subscriptions (migration 007)
-- ============================================================================
...
-- ============================================================================
-- 25. shipments — per-order shipment with carrier/tracking (migration 006)
-- ============================================================================
...
-- ============================================================================
-- 26. tracking_events — shipment event timeline (migration 006)
-- ============================================================================
...
-- ============================================================================
-- 27. returns — full return lifecycle (migration 006)
-- ============================================================================
...
-- ============================================================================
-- 28. return_items — line-level return quantities (migration 006)
-- ============================================================================
...
-- ============================================================================
-- 29. financial_ledger — source of truth for ALL money movement (migration 008)
-- ============================================================================
...
-- ============================================================================
-- 30. seller_balances — derived balances (recomputed from ledger) (migration 008)
-- ============================================================================
...
-- ============================================================================
-- 31. seller_payouts — payout requests (migration 008)
-- ============================================================================
...
-- ============================================================================
-- 32. platform_settings — key/value config (migration 010)
-- ============================================================================
...
-- ============================================================================
-- 33. notifications — in-app notifications (migration 010)
-- ============================================================================
...
-- ============================================================================
-- 34. audit_logs — append-only (migration 010)
-- ============================================================================
...
-- ============================================================================
-- 35. staff_profiles — department + granular permissions (migration 010)
-- ============================================================================
...
-- ============================================================================
-- 38. behavioral_events — durable event store (migration 011)
--     Append-only. Receives events from Convex realtime layer.
-- ============================================================================
...
-- ============================================================================
-- 39. event_flush_cursor — tracks Convex → Neon flush progress (migration 011)
-- ============================================================================
...
-- ============================================================================
-- 40. customer_profiles — derived customer intelligence (migration 015)
--     Rebuilt from behavioral_events + orders. Non-critical, rebuildable.
-- ============================================================================
...
-- ============================================================================
-- 41. customer_segments — derived segmentation rules (migration 015)
-- ============================================================================
